...
The `lustre-server` charm implements the `filesystem` endpoint, providing compatibility with the [`filesystem-client`](https://charmhub.io/filesystem-client) charm. Clients mount the Lustre filesystem by integrating a deployed `filesystem-client` with the `lustre-server` via Juju. TODO: link to how-to.
